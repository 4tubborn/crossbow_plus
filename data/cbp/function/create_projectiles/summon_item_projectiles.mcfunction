...
#tellraw @a {storage:cbp,nbt:"tmp_projectile_id"}
...
execute unless data storage cbp {"tmp_projectile_id":{"id":"minecraft:sand"}} as @n[type=falling_block,tag=item_projectiles,distance=..5,nbt={BlockState:{Name:"minecraft:sand"}}] run return run function cbp:create_projectiles/identify_special_entity
#tnt有对应方块所以falling_block不会变成沙子，上述判断失效，故单独做判断
execute if data storage cbp {"tmp_projectile_id":{"id":"minecraft:tnt"}} as @n[type=falling_block,tag=item_projectiles,distance=..5] run return run function cbp:create_projectiles/create_special_entity with storage cbp tmp_projectile_id
